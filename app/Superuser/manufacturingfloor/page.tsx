...
export default function AssemblyFloorPage() {
  ...
  const FLOOR_NAME = "Manufacturing_Floor";

  interface LineApiResponse {
    [key: string]: LineData;
  }

  // 1. Manufacturing Floor එකට අදාළ Lines ලබා ගැනීම
  useEffect(() => {
  ...
  // Floor එක සඳහා සමස්ත සංඛ්‍යාලේඛන ගණනය කිරීම
    ...
    <div className="bg-neutral-50 w-full min-h-screen p-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between sm:items-center mb-6 gap-4 rounded-2xl bg-linear-to-r from-slate-800 to-slate-700 p-6 shadow-md">
        <div className="flex items-center gap-3">
          ...
          <div>
            {/* මාතෘකාව Manufacturing Floor ලෙස නිවැරදි කර ඇත */}
            <h1 className="text-2xl font-extrabold text-white">Manufacturing Floor</h1>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 text-center text-slate-500 border border-slate-200 mb-8">No lines assigned to the Manufacturing Floor.</div>
      ) : (
