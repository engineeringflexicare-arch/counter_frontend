  ...
  return (
    <div className="flex flex-col bg-neutral-50 rounded-xs w-full min-h-screen p-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
        {/* lineId එක හිස් නැතිනම් පමණක් නම පෙන්වීම */}
        <h1 className="text-2xl text-slate-900 font-extrabold font-sans text-center">{lineId ? lineId.replaceAll("_", " ") : "Line"} Overview</h1>
      ...
      <div className="w-full">{lineId ? <ProductionTable lineId={lineId} date={selectedDate} /> : <p className="text-center text-slate-500 py-10">Loading line data...</p>}</div>
    </div>
  );
}
