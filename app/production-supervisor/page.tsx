...
function MachineCard({ machine }: { machine: InjectionMachineData }) {
    ...
    <div className="group w-full text-left rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition-all duration-150 hover:border-teal-300 hover:shadow-md hover:-translate-y-0.5 focus:outline-none focus-visible:ring-2 focus-visible:ring-teal-400">
      ...
      <div className="flex items-start justify-between gap-2 mb-4">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <p className="text-[10px] font-semibold uppercase tracking-widest text-slate-400">Machine No</p>
            <p className="font-mono text-base font-bold leading-tight text-slate-800">{machine.id}</p>
          </div>
        </div>
      ...
      <div className="mb-4 flex flex-wrap gap-3">
        <MetaChip icon={<Cpu className="h-3 w-3" />} label={machine.machineId || "No ESP32"} />
        <MetaChip icon={<Box className="h-3 w-3" />} label={`Mould: ${machine.mouldNumber || "—"}`} />
        <MetaChip icon={<Package className="h-3 w-3" />} label={machine.productCode || "—"} />
        {machine.cavities && machine.cavities > 1 && (
          <span className="inline-flex items-center rounded-md border border-slate-200 bg-slate-50 px-2 py-0.5 font-mono text-[11px] text-slate-600">Cav: {machine.cavities}</span>
        )}
        {machine.shift && (
          <span
            className={`inline-flex items-center rounded-md border px-2 py-0.5 text-[11px] font-medium ${
              machine.shift === "Day" ? "border-amber-200 bg-amber-50 text-amber-700" : "border-indigo-200 bg-indigo-50 text-indigo-700"
            }`}
          >
            {machine.shift} Shift
          </span>
...
export default function SupervisorInjectionDashboard() {
