...
import { UserPlus, Edit, Trash2, X, Shield, Search, Mail, User, ShieldAlert, ShieldCheck, ShieldQuestion } from "lucide-react";
...
const ROLE_STYLES: Record<string, { dot: string; badge: string; stripe: string; icon: React.ElementType }> = {
  Admin: { dot: "bg-violet-400", badge: "bg-violet-500/10 text-violet-300 ring-violet-500/30", stripe: "border-violet-500", icon: ShieldAlert },
  Supervisor: { dot: "bg-amber-400", badge: "bg-amber-500/10 text-amber-300 ring-amber-500/30", stripe: "border-amber-500", icon: ShieldQuestion },
  Operator: { dot: "bg-emerald-400", badge: "bg-emerald-500/10 text-emerald-300 ring-emerald-500/30", stripe: "border-emerald-500", icon: ShieldCheck },
};

const getRoleStyle = (role: string) => ROLE_STYLES[role] || ROLE_STYLES.Operator;
...
export default function UserManagement() {
  ...
  const [formData, setFormData] = useState({ name: "", employeeId: "", password: "", role: "Operator" });
  ...
  const filteredUsers = useMemo(() => {
    ...
    if (!q) return users;
    return users.filter((user) => user.name?.toLowerCase().includes(q) || user.employeeId?.toLowerCase().includes(q));
  }, [users, searchQuery]);

  // --- MODAL HELPERS ---
  const openEditModal = (user: UserData) => {
    setSelectedUser(user);
    setFormData({ name: user.name, employeeId: user.employeeId, password: "", role: user.role });
    setIsEditOpen(true);
  };
  ...
  const handleAddUser = async (e: React.FormEvent) => {
    ...
    const token = localStorage.getItem("token");
    try {
      await axios.post(`${API_BASE_URL}/api/users/add`, formData, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const handleEditUser = async (e: React.FormEvent) => {
    ...
    const token = localStorage.getItem("token");
    try {
      await axios.put(`${API_BASE_URL}/api/users/${selectedUser.id}`, formData, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const handleDeleteUser = async () => {
    ...
    const token = localStorage.getItem("token");
    try {
      await axios.delete(`${API_BASE_URL}/api/users/${selectedUser.id}`, { headers: { Authorization: `Bearer ${token}` } });
    ...
    <div className="min-h-screen bg-slate-950 p-6 md:p-10 font-sans text-slate-100">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 gap-6 border-b border-slate-800 pb-6">
          ...
          <div className="flex items-center gap-6">
            ...
            <button
              onClick={() => {
                setFormData({ name: "", employeeId: "", password: "", role: "Operator" });
                setIsAddOpen(true);
              }}
        ...
        {message && (
          <div
            className={`mb-4 px-4 py-3 rounded-lg font-mono text-sm border ${
        ...
        <div className="mb-4">
          <div className="relative max-w-md">
            ...
            <input
              type="text"
              placeholder="Search by name or employeeId
                    ..."
              value={searchQuery}
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-slate-800">
                  ...
                  <th className="p-4 font-mono font-medium text-xs uppercase tracking-widest text-slate-500">employeeId</th>
              ...
              <tbody>
                {loading && users.length === 0 ? (
                  <tr>
                    <td colSpan={4} className="p-10 text-center text-slate-500 font-mono text-sm animate-pulse">
                      Loading users...
                    </td>
                  </tr>
                  ...
                  filteredUsers.map((user) => {
                    ...
                    return (
                      <tr key={user.id} className={`border-b border-slate-800 border-l-2 ${style.stripe} hover:bg-slate-800/50 transition-colors`}>
      ...
      {(isAddOpen || isEditOpen) && (
        <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 w-full max-w-md rounded-2xl border border-slate-800 shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className="p-6 border-b border-slate-800 flex justify-between items-center">
              <div>
                <div className="flex items-center gap-2 text-cyan-400 text-xs font-mono uppercase tracking-[0.2em] mb-1">
                  <Shield size={12} />
                  {isEditOpen ? "Edit Record" : "New Record"}
                </div>
            ...
            <form onSubmit={isEditOpen ? handleEditUser : handleAddUser} className="p-6 space-y-4">
              ...
              <div>
                <label className="flex items-center gap-2 text-xs font-mono uppercase tracking-wider text-slate-400 mb-2">
                  <Mail size={14} /> employeeId Address
                </label>
                <input
                  required
                  type="employeeId
              ...
              {/* Password field only required when adding a new user */}
              {isAddOpen && (
                <div>
      ...
      {isDeleteOpen && (
        <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 w-full max-w-sm rounded-2xl border border-slate-800 shadow-2xl p-6 text-center animate-in fade-in zoom-in-95 duration-200">
            ...
            <p className="text-slate-400 text-sm mb-6">
              You&apos;re about to permanently remove <strong className="text-slate-100 font-mono">{selectedUser?.name}</strong> from the system. This action cannot be undone.
            </p>
